...
float PPMcalc(float val, uint8_t sensor, float sens_code){
  ...
  if (sensor == 1){
    g2_int = ((val-2.560)*1000);
    if (g2_int<0){ g2_int = 0;} 
  }
  else if (sensor == 1){
    g1_int =((val-2.560)/0.029);
  ...
  if((rx_char == '\r')||(i>7)){
    ...
    for(j=0; j<strlen(uart_buff); j++){    
      ...
      if(uart_buff[j] == 't'){
        ...
        if (cli_en) {uart_puts("ADC1(H2S): ");}
        uart_puts(adc1_uart);
        uart_buff[j]=' ';
        new_line=1;
      }
      else if(uart_buff[j] == 'n'){
        if(cli_en) {uart_puts("ADC2(CO): ");}
        uart_puts(adc2_uart);
        uart_buff[j]=' ';
        new_line=1;
      }
      else if(uart_buff[j] == 'r'){
        if (cli_en){uart_puts("PPM(H2S): ");}
        uart_puts(gas1_uart);
        uart_buff[j]=' ';
        new_line=1;
      }
      else if(uart_buff[j] == 'p'){
        if (cli_en){ uart_puts("PPM(CO): ");}
        uart_puts(gas2_uart);
        uart_buff[j]=' ';
        new_line=1;
      }
      else if(uart_buff[j] == 'j'){
        if (cli_en){ uart_puts("int(CO): ");}
        uart_puts(g2_int_uart);
        uart_buff[j]=' ';
        new_line=1;
      }
      else if(uart_buff[j] == 'y'){
        if (cli_en){ uart_puts("int(H2S): ");}
...
int main(){
  ...
  while(1){
    ...
    if (adc2_count == 9){ // average of 10 sample voltages
      ...
      gas_buf = PPMcalc(adc2_f_avg, 2, 3.3);
    ...
    if (adc1_count == 9){ //average of 10 sample voltages
      ...
      gas_buf = PPMcalc(adc1_f_avg, 1, 182.78);
